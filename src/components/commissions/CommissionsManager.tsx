...
const CommissionsManager: React.FC = () => {
  ...
  const { commissionsData, loading, error } = useCommissions(startDate, endDate);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="text-sm text-gray-500">
          Suivi des commissions perçues par compte
        </div>
      </div>
          ...
          <div className="flex items-center space-x-2 md:space-x-3 min-w-0 flex-shrink">
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 break-words">
                {commissionsData.length} compte{commissionsData.length > 1 ? 's' : ''} avec commission{commissionsData.length > 1 ? 's' : ''}
              </p>
            </div>
          </div>
      ...
      {(startDate || endDate) && showFilterSection && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 max-w-full min-w-0 overflow-hidden">
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-1 w-full sm:w-auto max-w-full min-w-0">
            <div className="flex items-center space-x-2 min-w-0 flex-1">
              <Calendar className="h-5 w-5 text-green-600 flex-shrink-0" />
              <div className="min-w-0">
                <h4 className="text-sm font-medium text-green-900">Période active</h4>
                <p className="hidden md:block text-xs text-green-700 break-words">
                  {typeof dateRangeText === 'object' ? dateRangeText.desktop : dateRangeText}
                </p>
                <p className="md:hidden text-xs text-green-700 break-words">
                  {typeof dateRangeText === 'object' ? dateRangeText.mobile : dateRangeText}
                </p>
              </div>
            </div>
            <button
              onClick={clearDateFilter}
              className="p-2 text-green-600 hover:text-green-800 hover:bg-green-100 rounded-lg transition-colors flex-shrink-0"
